...
class DashboardController {
    ...
    def admin() {
        ...
        try {
            render(view: '/dashboard/admin', model: [
                    ...
                    totalInterns: Intern.count() ?: 0,
                    activeInterns: Intern.countByStatus('ACTIVE') ?: 0,
                    totalSupervisors: Supervisor.count() ?: 0,
                    pendingReports: WeeklyReport.countByStatus('SUBMITTED') ?: 0,
                    overdueTasks: Task.countByStatus('OVERDUE') ?: 0,
                    recentUsers: User.list(max: 5, sort: 'dateCreated', order: 'desc') ?: [],
                    recentInterns: Intern.list(max: 5, sort: 'dateCreated', order: 'desc') ?: []
            ])
        } catch (Exception e) {
            println "Error in admin dashboard: ${e.message}"
            render(view: '/dashboard/admin', model: [error: "Error loading dashboard"])
        }
    }
    ...
    private List getSupervisorTasks(Supervisor supervisor) {
        try {
            return Task.createCriteria().list(max: 10) {
                intern { eq('supervisor', supervisor) }
                order('dueDate', 'asc')
            } ?: []
        } catch (Exception e) {
            return []
    ...
    private Map getTaskStatistics(Intern intern) {
        if (!intern) return [total: 0, completed: 0, pending: 0, inProgress: 0, overdue: 0, completionRate: 0]
        try {
            def total = Task.countByIntern(intern) ?: 0
            def completed = Task.countByInternAndStatus(intern, 'COMPLETED') ?: 0
            def pending = Task.countByInternAndStatus(intern, 'PENDING') ?: 0
            def inProgress = Task.countByInternAndStatus(intern, 'IN_PROGRESS') ?: 0
            def overdue = Task.countByInternAndStatus(intern, 'OVERDUE') ?: 0
            def completionRate = total > 0 ? Math.round((completed / total * 100)) : 0
            return [total: total, completed: completed, pending: pending, inProgress: inProgress, overdue: overdue, completionRate: completionRate]
        } catch (Exception e) {
            return [total: 0, completed: 0, pending: 0, inProgress: 0, overdue: 0, completionRate: 0]
    ...
    private Map getReportStatistics(Intern intern) {
        if (!intern) return [total: 0, approved: 0, submitted: 0, underReview: 0, totalHours: 0, avgHours: 0]
        try {
            ...
            def avgHours = total > 0 ? Math.round(totalHours / total) : 0
            return [total: total, approved: approved, submitted: submitted, underReview: underReview, totalHours: totalHours, avgHours: avgHours]
        } catch (Exception e) {
            return [total: 0, approved: 0, submitted: 0, underReview: 0, totalHours: 0, avgHours: 0]
        }
    }
    ...
    private Map getEvaluationStatistics(Intern intern) {
        ...
        try {
            def evaluations = Evaluation.findAllByIntern(intern) ?: []
            def totalScore = evaluations.sum { it.overallScore ?: 0 } ?: 0
            def averageScore = evaluations.size() > 0 ? Math.round(totalScore / evaluations.size()) : 0
            return [totalEvaluations: evaluations.size(), averageScore: averageScore]
        } catch (Exception e) {
            return [totalEvaluations: 0, averageScore: 0]
        }
