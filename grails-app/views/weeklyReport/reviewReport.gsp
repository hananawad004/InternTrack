...
<script>
    function updateReportStatus(status) {
        ...
        // التحقق من وجود تعليقات للحالات المهمة
        if (status === 'REJECTED' && !comments) {
            alert('Please provide comments when rejecting a report');
            $('#comments').focus();
            return;
        }

        if (status === 'UNDER_REVIEW' && !comments) {
            alert('Please provide comments when marking as under review');
            $('#comments').focus();
            return;
        }

        // تحديد نص الإجراء المناسب
        let actionText = '';
        if (status === 'APPROVED') {
            actionText = 'approve';
        ...
        // استخدام concatenation بدلاً من template literals
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    localStorage.removeItem('report_${report.id}_comments');
                    // Redirect back to review list
                    window.location.href = '${createLink(controller: "weeklyReport", action: "review")}';
                } else {
                    alert('Error: ' + response.message);
